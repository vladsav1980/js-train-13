...
function searchParamsURL(url) {
  ...
  const urlSearchParams = Object.fromEntries(newUrl.searchParams.entries());
  let params = new Map();
  for (let param in urlSearchParams) {
    params.set(param, urlSearchParams[param]);
  }
  return params;
}
...
function checkURLParameters(url, params) {
  ...
  for (const key of params) {
    checkedParams[key] = newUrl.searchParams.has(key);
  }
...
function manipulateQuery(url, options) {
  ...
  if (options.has("delete")) {
    for (const param of options.get("delete")) {
